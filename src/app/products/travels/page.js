...
const Travels = async () => {
  const [travelsData] = await cartcrazeDB.query("SELECT * FROM travels");
        ...
        <div className="container mx-auto px-4">
          <div className="text-center">
            ...
            <p className="text-xl text-gray-300 max-w-2xl mx-auto">
              Gear up for your next adventure with our premium collection of
              travel accessories and essentials designed to make every journey
            ...
            {travelsData.map((travel) => (
              <Link
                ...
                href={`/${encodeURIComponent(
                  travel.title.toLowerCase().replace(/\s+/g, "-")
                )}?id=${travel.id}&category=${encodeURIComponent(
                  travel.category
                )}`}
                className="block"
              >
                    ...
                    <div>
                      {travel.stock > 0 ? (
                        <span className="text-green-400 text-sm">
                          ✓ In Stock ({travel.stock} available)
                        </span>
                      ) : (
